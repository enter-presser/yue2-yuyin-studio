# Yue 2.0 · Token Monster

基于 YuE2 的非官方中文音乐创作工作台：填写音乐风格和歌词，生成歌曲、试听，并从已有版本继续修改。

**[中文使用说明与启动方法 →](workbench/README.md)** · [工作台源码](workbench/)

- 当前为直接输入版，已移除外部 AI 创作助手、聊天和 API 配置入口；无需配置创作大模型 API。
- 保留中文手稿编辑、保存与撤销、分阶段生成、任务状态、播放器、音频下载和版本管理。
- 高级设置仅显示生成方式、种子和 ABC 乐谱。可以粘贴已有乐谱进行修改，或以新风格重新演绎。
- 启动时复用已验证的本地模型；缺失时优先接入平台共享文件，未挂载或不匹配则自动从 HF-Mirror 下载。支持暂停、断点续传与文件校验。
- 本仓库不包含模型权重、密钥、用户配置或私人作品。`workbench/model-assets` 仅包含配置、分词器、许可证及用于验证和转换的文件头。
- 保留官方源代码、历史和许可证，与官方项目没有隶属关系。上游算法更新与工作台发布分开验证。
