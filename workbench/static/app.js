...
function renderModels(s){
 ...
 const active=['checking','downloading','verifying','cancelling'].includes(s.state);
 $('model-banner').hidden=s.ready;
 $('model-banner-title').textContent=s.state==='missing'?'首次使用，先准备音乐模型':s.state==='ready'?'音乐模型已就绪':s.state==='failed'?'模型下载需要重试':s.state==='paused'?'模型下载已暂停':'正在准备音乐模型';
 $('model-banner-detail').textContent=s.detail;
 $('models-banner-open').textContent=active?'查看进度':'下载模型';
 $('model-status').textContent=s.detail;
 ...
 $('model-download').textContent=s.ready?'模型已就绪':s.state==='paused'||s.state==='failed'?'继续下载':'下载音乐模型';
